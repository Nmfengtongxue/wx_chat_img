...
import { flushScreenshotLayout } from './screenshotPrepare'
...
function unlockOverflowChain(el: HTMLElement): RestoreFn {
  ...
  return () => {
    saved.forEach(({ node, overflow, overflowY, maxHeight }) => {
...
export async function captureElementToPng(
  ...
  try {
    await flushScreenshotLayout(element)
    ...
    // 再测量一次，确保气泡 SVG 已按最新文字尺寸更新
    await flushScreenshotLayout(element)
    const finalSize = measureExportSize(element, long && fitContent)
